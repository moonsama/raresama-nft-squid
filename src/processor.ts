...
const processor = new SubstrateBatchProcessor()
    ...
    .addEvmLog(config.FACTORY_ADDRESS, {
        filter: [collectionFactory.events['CollectionAdded(uint256,bytes32,address)'].topic],
    })
...
async function handleEvmLog(ctx: BatchContext<Store, unknown>, block: SubstrateBlock, event: EvmLogEvent) {
  ...
  if (contractAddress === config.FACTORY_ADDRESS 
    && event.args.topics[0] === collectionFactory.events["CollectionAdded(uint256,bytes32,address)"].topic) {
      await handleNewContract(ctx, block, event)
  } 
  else if (await isKnownContract(ctx.store, contractAddress)
